...
final parcoursRecordingNotifierProvider =
    StateNotifierProvider<ParcoursRecordingNotifier, ParcoursRecordingState>((ref) {
...
class ParcoursRecordingNotifier extends StateNotifier<ParcoursRecordingState> {
  ...
  Future<void> startRecording() async {
    ...
    _locationSubscription = _ref.listen<LocationState>(locationNotifierProvider, (_, next) {
      if (next.locationData != null) {
        state = state.copyWith(
          recordedLocations: List.from(state.recordedLocations)..add(next.locationData!),
        );
      }
    });
