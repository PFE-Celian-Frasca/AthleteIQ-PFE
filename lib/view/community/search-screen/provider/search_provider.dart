...
import 'package:athlete_iq/view/community/search-screen/provider/group_search_state.dart';
import 'package:athlete_iq/view/community/search-screen/provider/user_search_state.dart';

final groupSearchProvider = StateNotifierProvider<GroupNotifier, GroupSearchState>((ref) {
  final groupServiceProvider = ref.watch(groupService);
  return GroupNotifier(groupServiceProvider);
});

final userSearchProvider = StateNotifierProvider<UserNotifier, UserSearchState>((ref) {
...
class GroupNotifier extends StateNotifier<GroupSearchState> {
  ...
  void _loadGroups() {
    _groupsSubscription = _groupService.listPublicGroupsStream().listen((groups) {
